...
class CmsTagLib
{
    ...
    public function pre($data)
    {
        ...
        $result = model('cms/Cms')->getContent(getCategory($data['catid'], 'modelid'), "catid = " . $data['catid'] . " AND status = 1 AND id < " . $data['id'], false, 'catid,id,title,url', "id desc");
        if (!$result) {
            $result['title']  = $msg;
    ...
    public function next($data)
    {
        ...
        //是否新窗口打开
        $target = !empty($data['target']) ? ' target=_blank ' : '';
        $result = model('cms/Cms')->getContent(getCategory($data['catid'], 'modelid'), "catid = " . $data['catid'] . " AND status = 1 AND id > " . $data['id'], false, 'catid,id,title,url');
        if (!$result) {
            $result['title']  = $msg;
            $result['url']    = 'javascript:alert("' . $msg . '");';
            $result['target'] = $target;
