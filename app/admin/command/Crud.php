...
class Crud extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        //最终将生成的文件路径
        $javascriptFile = app()->getRootPath() . 'public' . DS . 'assets' . DS . 'js' . DS . 'backend' . DS . $controllerBaseName . '.js';
        ...
        if ($delete) {
            $readyFiles = [$controllerFile, $modelFile, $validateFile, $addFile, $editFile, $indexFile, $recyclebinFile, $javascriptFile];
        ...
        try {
                ...
                if ($v['COLUMN_KEY'] != 'PRI' && !in_array($field, $this->reservedField) && !in_array($field, $this->ignoreFields)) {
                    ...
                    if ($inputType == 'select') {
                        ...
                        if ($this->isMatchSuffix($field, $this->selectpageSuffix)) {
                            ...
                            $tableInfo = null;
                            try {
                                $tableInfo = Db::name($selectpageTable)->getTableInfo();
            ...
            //JS最后一列加上操作列
            $javascriptList[] = str_repeat(" ", 15) . "{fixed: 'right',width:90,field: 'operate', title: '操作',templet: Table.formatter.tool,operat: ['edit','delete']}";
    ...
    protected function getJsColumn($field, $datatype = '', $extend = '', $itemArr = [])
    {
        ...
        if ($formatter) {
            $html .= ", templet: Table.formatter." . $formatter . "}";
        } else {
            $html .= "}";
        }
        return $html;
...
<div class="layui-col-xs4">
            ...
            <button type="button" id="faupload-{$field}" class="layui-btn faupload" data-input-id="c-{$field}"{$uploadfilter}{$multiple}{$preview}><i class="iconfont icon-upload-line"></i> 上传</button><button type="button" id="fachoose-{$field}" class="layui-btn fachoose" data-input-id="c-{$field}"{$selectfilter}{$multiple}><i class="iconfont icon-list-unordered"></i> 选择</button>
            {$previewcontainer}
EOD;
    }
...
EOD;
    ...
    protected function getParseNameData($module, $name, $table, $type)
    {
        ...
        $moduleDir      = app()->getBasePath() . $module . DS;
