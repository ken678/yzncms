...
class Yzn extends Taglib
{
    ...
    public function tagTemplate($tag, $content)
    {
        ...
        //不是直接指定模板路径的
        if (false === strpos($templateFile, Config::get('view.view_suffix'))) {
            $templateFile = TEMPLATE_PATH . $theme . '/' . $templateFile . Config::get('view.view_suffix');
        } else {
    ...
    public function tagYzn($tag, $content)
    {
        ...
        $parseStr .= 'else: ';
        $parseStr .= '$' . $module . 'TagLib =  \think\Container::pull("\\\\addons\\\\' . $module . '\\\\taglib\\\\' . ucwords($module) . 'TagLib");';
