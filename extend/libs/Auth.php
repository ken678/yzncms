...
class Auth
{
    ...
    protected function getAuthList(int $uid)
    {
        ...
        //读取用户组所有权限规则
        $rules = Db::name($this->config['AUTH_RULE'])->where($where)->field('id,parentid,condition,icon,name,title,ismenu')->select();
        //循环规则，判断结果。
        $rulelist = [];
        if (in_array('*', $ids)) {
            $rulelist[] = "*";
    ...
    protected function getUserInfo(int $uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $userinfo[$uid] = Db::name($this->config['auth_user'])->find($uid);
        }
        return $userinfo[$uid];
    }
