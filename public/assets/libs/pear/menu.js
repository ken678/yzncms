define(['jquery', 'layui'], function($, layui) {
	...
	function createMenu(option) {
		...
		$.each(option.data, function (i, item) {
			...
			if (item.type == 0) {
				// 创 建 目 录 结 构
				content += '<a ' + item.extend + ' href="javascript:;" menu-type="' + item.type + '" menu-id="' + item.id + '" href="' + href +
					'" ' + target + '><i class="' + item.icon + '"></i><span>' + item.title +
					'</span></a>';
			} else if (item.type == 1) {
				content += '<a ' + item.extend + ' class="' + className + '" menu-type="' + item.type + '" menu-url="' + item.href + '" menu-id="' +
					item.id +
	...
	function createMenuAndControl(option) {
		...
		$.each(option.data, function (i, item) {
			...
			var target = "";
			var href = "javascript:;";
			if (item.openType == "_blank" && item.childlist.length == 0) {
				href = item.href;
				item.href = '';
				target = "target='_blank'";
			}
			if (i === option.defaultMenu) {
				...
				controlPe += '<li class="layui-nav-item"><a class="pe-title" href="javascript:;" ><i class="icon iconfont icon-list-unordered"></i>&nbsp;选择模块</a>';
				...
				controlItem = '<li  pear-href="' + item.href + '" pear-title="' + item.title + '" pear-id="' + item.id +
					'" class="layui-nav-item"><a ' + item.extend + ' href="'+ href +'" ' + target + '>' + item.title + '</a></li>';
